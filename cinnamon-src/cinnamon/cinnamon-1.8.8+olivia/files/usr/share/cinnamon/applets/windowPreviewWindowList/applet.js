...
function HoverController(actor, menu){
    this._init(actor, menu);
}

HoverController.prototype={
    _init: function(actor, menu){
    ...
    _onEnter: function(){
        ...
        Mainloop.timeout_add(HOVER_TIME, Lang.bind(this, this.open));
    },

    _onLeave: function(){
        this.shouldOpen = false;

        Mainloop.timeout_add(HOVER_TIME, Lang.bind(this, this.close));
    },
...
function WindowPreview(actor, metaWindow, orientation){
...
WindowPreview.prototype={
	...
	this.metaWindow = metaWindow;
        ...
        this.actor = new St.Bin({style_class: "popup-menu-boxpointer"});
        this.container = new St.BoxLayout({vertical: true, style_class: "popup-menu"}); //Used two layers of actors in order to apply two style classes
        this.container.style = "padding: 6px; spacing: 5px";
        this.actor.add_actor(this.container);

        this.label = new St.Label();
        this.thumbnailBin = new St.Bin({width: 200, height: 150});

        this.container.add_actor(this.label);
        this.container.add_actor(this.thumbnailBin);

        this.metaWindow.connect("notify::title", Lang.bind(this, function(){this.label.text = this.metaWindow.get_title();}));
    ...
    _refreshDisplay: function(open){
        ...
        if (mutterWindow){
            ...
            // Thumbnail size (width, height) is (200, 150)
            let [width, height] = windowTexture.get_size();
            let scale = Math.min(1.0, 200/width, 150/height);
            this.windowThumbnail = new Clutter.Clone({ source: windowTexture,
                                                       reactive: true,
                                                       width: Math.floor(width * scale),
                                                       height: Math.floor(height * scale)});
            this.label.text = this.metaWindow.get_title();
            this.thumbnailBin.set_child(this.windowThumbnail);

            // Position box
            let parentActorWidth = this._parentActor.get_width();
            let parentActorHeight = this._parentActor.get_height();
            let selfActorWidth = this.actor.get_width();
            let selfActorHeight = this.actor.get_height();
            let [parentX, parentY] = this._parentActor.get_position();

            let parent = this._parentActor.get_parent();
            let offsetX = 0, offsetY = 0;
            while (parent){
                if (parent.get_x() + 1 > 0){
                    offsetX += parent.get_x();
                    offsetY += parent.get_y();
                }
                parent = parent.get_parent();
            }
            let y1
            if (this.orientation == St.Side.BOTTOM){
                y1 = parentY - selfActorHeight + offsetY;
            } else {
                y1 = parentY + parentActorHeight + offsetY;
            }
            let x1 = parentX + parentActorWidth/2 - selfActorWidth/2 + offsetX;
            this.actor.set_position(Math.round(x1), Math.round(y1));
        }
    },
...
AppMenuButton.prototype = {
    _init: function(applet, metaWindow, animation, orientation) {
        ...
        this._updateCaptionId = this.metaWindow.connect("notify::title", Lang.bind(this, function() {
            this._label.set_text(this.getDisplayTitle())
        }));
        ...
        // Set up the window preview
        this.preview = new WindowPreview(this.actor, this.metaWindow, orientation);
        this.controller = new HoverController(this.actor, this.preview);
    },
