...
function getapppkg()
{
    ...
    if [ "$T" ]; then
        ...
        cd $(gettop)
        rsync -av --delete --progress $RAWPREAPPADDRESS $OUT/$PREAPP
        addrepository $OUT/$PREAPP/gir1.2-gtop-2.0_2.28.4-3_i386.deb || return 1
        addrepository $OUT/$PREAPP/fcitx-frontend-qt5_0.1.1-0~22~ubuntu13.04.1_i386.deb || return 1
        addrepository $OUT/$PREAPP/fcitx-libs-qt5_0.1.1-0~22~ubuntu13.04.1_i386.deb || return 1
    else
...
#only server can commit squashfs
function _msquashfs()
{
    BUITSTEP=0
    ISONSERVER=0
    ...
    if [ "$T" ]; then
        ...
        if [ $BUITSTEP -le 15 ] ; then
            ...
            echo "mksquashfs $RAWSQUASHFSNAME succeed!"

            if [ $ISONSERVER -eq 1 ]; then
                echo "rsync to jenkins"
                if [ -e $SERVERPATH/$RAWSQUASHFSNAME ]; then
                    rm -rf $SERVERPATH/$RAWSQUASHFSNAME
                fi
                rsync -av --progress $OUT/$RAWSQUASHFSNAME $SERVERPATH
                echo "rsync $RAWSQUASHFSNAME succeed!"
            fi
        fi
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
...
#only server can commit devsaved repository
function _mrepository()
{
    BUITSTEP=0
    ISONSERVER=0
    if [ -e $BUILDOSSTEP ] ; then
...
#            getprepkg || return 1
        ...
        if [ $BUITSTEP -le 25 ] ; then
            ...
            createlink || return 1
            mall || return 1
            echo "make $APPOUT $REPODIRNAME succeed."
        fi
        if [ $BUITSTEP -le 26 ] ; then
            ...
            if [ $ISONSERVER -eq 1 ]; then
                ...
                echo "rsync $APPOUT $REPODIRNAME succeed!"
            fi
        fi
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    ...
    if [ "$T" ]; then
        ...
        if [ $BUITSTEP -le 30 ] ; then
            ...
            if [ $ISONSERVER -eq 1 ] ; then
                ...
                mkserverwktdir $SERVERPATH $SERVERAPPPATH
            else
                rsync -av --progress $WSHAREPATH/$RAWSQUASHFSNAME $OUT/$RAWSQUASHFSNAME
                rsync -av --delete --progress $RAWPREAPPADDRESS $OUT/$PREAPP
                if [ ! -d $OUT/$APPOUT ]; then
                    rsync -av --delete --progress $WSHAREPATH/$APPOUT $OUT/$APPOUT
                fi
                if [ ! -d $REPOSITORY ]; then
                    rsync -av --delete --progress $WSHAREPATH/$REPODIRNAME $OUT/$REPODIRNAME
                fi
                if [ -d $OUT/out ]; then
                    cclean out -y
                fi
